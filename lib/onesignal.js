...
const { batch, db, meta, nconf, translator, user, utils } = require('./nodebb');
...
OneSignal.push = async function (data) {
	...
	try {
		...
		for (const [language, uids] of languages.entries()) {
			setTimeout(() => {
				batch.processArray(Array.from(uids), async (uidsPart) => {
					await OneSignal.pushToUids(
						uidsPart,
						notification,
						language,
						{
